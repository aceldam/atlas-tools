...
namespace AtlasApp {
    export class MenuTools {
        ...
        injectMenuItem(): void {
            let $menuContainer = $("#observation-modal > md-toolbar > div").append('<md-menu md-position-mode="target-right target" class="md-menu ng-scope _md">');

            let $button = $menuContainer.append('<button id="maps-button" style="font-size: 20px;" class="md-icon-button md-button md-ink-ripple">G</button>');
            ...
            $.getJSON("https://svampe.databasen.org/api/observations/" + id, (data: Observation) => {
        ...
        getClipboardText(observation: Observation): string {
            let result = observation.PrimaryDetermination.Taxon.acceptedTaxon.Vernacularname_DK.vernacularname_dk;
